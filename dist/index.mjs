...
async function detectCardBounds(buffer) {
  try {
    const { data, info } = await sharp3(buffer).greyscale().raw().toBuffer({ resolveWithObject: true });
...
async function detectAndCropFace(buffer) {
  try {
    ...
    let width, height;
    if (cardBounds) {
      cardBuffer = await sharp3(buffer).extract(cardBounds).toBuffer();
    ...
    const regionBuffer = await sharp3(cardBuffer).extract({
      left: 0,
      top: 0,
      width: regionWidth,
      height: regionHeight
    ...
    const croppedBuffer = await sharp3(regionBuffer).extract({
      left: crop.x,
      top: crop.y,
      width: crop.width,
      height: crop.height
...
async function cropCardWithGemini(imageBuffer) {
  ...
  try {
    const metadata = await sharp3(imageBuffer).metadata();
    if (!metadata.width || !metadata.height) return null;
...
async function cropRegion(buffer, bbox, imgW, imgH) {
  ...
  return sharp3(buffer).extract({ left, top, width, height }).toBuffer();
}
...
Return ONLY valid JSON, nothing else.`;
    ...
    const photo = await sharp3(imageBuffer).extract({ left, top, width, height }).resize(256, 256, { fit: "cover" }).jpeg({ quality: 92 }).toBuffer();
...
async function cropToFrontCard(imageBuffer) {
  try {
    ...
    let w, h;
    if (bounds) {
      workBuffer = await sharp3(imageBuffer).extract(bounds).toBuffer();
      w = bounds.width;
      h = bounds.height;
    } else {
      const meta = await sharp3(imageBuffer).metadata();
    ...
    if (h / w > ASPECT_RATIO_THRESHOLD) {
      ...
      workBuffer = await sharp3(workBuffer).extract({ left: 0, top: 0, width: w, height: halfH }).toBuffer();
      const frontBounds = await detectCedulaBounds(workBuffer);
      if (frontBounds) {
        workBuffer = await sharp3(workBuffer).extract(frontBounds).toBuffer();
      }
    }
    return workBuffer;
  } catch {
...
async function cropPhotoFromImage(buffer, bbox) {
  try {
    ...
    let workingHeight;
    if (cedulaBounds) {
      workingBuffer = await sharp3(buffer).extract(cedulaBounds).toBuffer();
    ...
    if (aspectRatio > ASPECT_RATIO_THRESHOLD) {
      ...
      workingBuffer = await sharp3(workingBuffer).extract({ left: 0, top: 0, width: workingWidth, height: halfHeight }).toBuffer();
      workingHeight = halfHeight;
      const frontBounds = await detectCedulaBounds(workingBuffer);
      if (frontBounds) {
        workingBuffer = await sharp3(workingBuffer).extract(frontBounds).toBuffer();
    ...
    const croppedBuffer = await sharp3(workingBuffer).extract({ left: safeLeft, top: safeTop, width: safeWidth, height: safeHeight }).resize(256, 256, { fit: "cover" }).jpeg({ quality: 92 }).toBuffer();
    if (croppedBuffer.length < 5e3) {
      return null;
    }
    return croppedBuffer.toString("base64");
...
async function findCardRegions(imageBuffer) {
  const { data, info } = await sharp3(imageBuffer).greyscale().raw().toBuffer({ resolveWithObject: true });
  const rowDarkness = [];
  const rowVariance = [];
  for (let y = 0; y < info.height; y++) {
...
async function detectAndSplitCompositeCedula(imageBuffer, mimetype, model = "gemini") {
  ...
  let backBuf;
  if (regions) {
    frontBuf = await sharp3(imageBuffer).extract(regions.front).toBuffer();
    backBuf = await sharp3(imageBuffer).extract(regions.back).toBuffer();
  } else {
    const halfHeight = Math.round(imgHeight / 2);
    frontBuf = await sharp3(imageBuffer).extract({ left: 0, top: 0, width: imgWidth, height: halfHeight }).toBuffer();
    backBuf = await sharp3(imageBuffer).extract({ left: 0, top: halfHeight, width: imgWidth, height: imgHeight - halfHeight }).toBuffer();
  }
...
async function generateThumbnailFromImage(buffer) {
  try {
    return await sharp3(buffer).resize(THUMB_WIDTH, null, { withoutEnlargement: true }).jpeg({ quality: THUMB_QUALITY, mozjpeg: true }).toBuffer();
  } catch {
    return null;
  }
}
